"""Harbor adapter that installs and runs pagentv4 in task containers."""
...
class PagentV4Agent(BaseInstalledAgent):
    """Run pagentv4 with its inplace backend in a Harbor task container."""
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if not (self.source_root / "pyproject.toml").is_file():
            raise ValueError(
                "PagentV4Agent source_root must point to the pagent repository"
            )
